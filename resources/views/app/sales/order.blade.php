...
@section("content")
    @if (session('order_succes'))
        <div class="alert alert-success" style="font-size: 0.9em;">
            {!! session('order_succes') !!}
        </div>
    @endif
    @if (session('order_process_error'))
        <div class="alert alert-danger" style="font-size: 0.9em;">
            {!! session('order_process_error') !!}
        </div>
    @endif

    {!! BootForm::openHorizontal($columnSizes)->post()->action( route('u_order_trait')) !!}
    <div class="row" style="background-color: #fff;padding: 30px;">
        ...
        <div class="col-md-3" style="padding-left: 20px;border-left: 1px solid #EEE;">
            <button style="margin-right: 5px" id="btnAdd" type="submit" class="btn btn-primary">ADD</button>
            <button style="margin-right: 5px" id="btnCreate" type="submit" class="btn btn-primary">CREATE</button>
